...
prom=StringVar()
def conexionBBDD():
   miConexion=sqlite3.connect("escuela.db")
...
def seleccionarUsandoClick(event):
    ...
    Curso.set(tree.item(item,"values")[1])
    notaPrimer.set(tree.item(item,"values")[2])
    notaSegunda.set(tree.item(item, "values")[3])
    notaTercera.set(tree.item(item, "values")[4])
    prom.set(tree.item(item, "values")[5])
...
def actualizar():
    ...
    try:
        ...
        miCursor.execute("UPDATE alumno SET dni=?, NOMBRE=?,CURSO=?,NOTAPRIMERA=?, NOTASEGUNDA=?, NOTATERCERA=?, PROMEDIO=? WHERE dni="+dni.get(),(datos))
        miConexion.commit()
    except:
    ...
    mostrar()
def borrar():
     miConexion=sqlite3.connect("escuelad.db")            
     miCursor=miConexion.cursor()
     try:
