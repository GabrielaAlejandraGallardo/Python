...
def crear():
  ...
  ##  miConexion=sqlite3.connect("base")
    ##miCursor=miConexion.cursor()
    try:
        datos=miNombre.get(),miCargo.get(),miSalario.get(),an.get()
...
def mostrar():
    ...
    try:
        miCursor.execute("SELECT NOMBRE,CARGO,SALARIO*an,an FROM empleado")   
        for row in miCursor:
...
def actualizar():
    ...
    try:
        datos=miNombre.get(),miCargo.get(),miSalario.get(),an.get()
        miCursor.execute("UPDATE empleado SET NOMBRE=?,CARGO=?,SALARIO=?,an=? WHERE ID="+miId.get(),(datos))
        miConexion.commit()
    except:
...
l5=Label(root,text="antiguedad")
l5.place(x=400,y=40)
e5=Entry(root,textvariable=an, width=10)
e5.place(x=450,y=40)
